...
class RandomNet(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        return torch.rand(batch_size, NUMBER_CLASSES).to(x.device)
...
class ConstantNet(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        return torch.full((batch_size, NUMBER_CLASSES), self.constant).to(x.device)

class DrivingNet(Model):
    def __init__(self, model_name, criterion, metric, device,
                 optimizer_name="adam", lr_scheduler="sqrt", initial_lr=1e-3, epoch_size=1):
        super(DrivingNet, self).__init__()
        if model_name == "FADNet":
            self.net = FADNet().to(device)
        elif model_name == "FADNet_plus":
            self.net = FADNet_plus().to(device)
        elif model_name == "ADTVNet":
        ...
        elif model_name == "RandomNet":
            self.net = RandomNet().to(device)
        elif model_name == "ConstantNet":
